...
public class Img4bitUtil {
	...
	public static VramImg toVramImg(BufferedImage img, PixelConverter cb){
	...
	public static List<BufferedImage> splitToTiles(BufferedImage src, int tileVramW, int tileH){
		...
		for(int y=0;y<src.getHeight();y+=tileH){
			for(int x=0;x<src.getWidth();x+=tileVramW*4){
				tiles.add(src.getSubimage(x, y, tileVramW*4, tileH));
			}
		}
		return tiles;
	}
	...
	public static List<VramImg> splitToTiles(BufferedImage src, int tileVramW, int tileH, Palette pal){
		...
		for(int y=0;y<src.getHeight();y+=tileH){
			for(int x=0;x<src.getWidth();x+=tileVramW*4){
				BufferedImage sub = src.getSubimage(x, y, tileVramW*4, tileH);
	 ...
	 * @param tiles
	 * @param column
	 * @param marginRight joint pic's marginRight
	 * @param bgColor
	 * @return
	 */
