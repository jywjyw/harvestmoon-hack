...
public class SectorErrorCorrection {
    ...
    /** Generate sector EDC.*/
    public static byte[] generateErrorDetectionAndCorrection(byte[] data, 
                                                           int iStart, int iEnd)
    {
        long edc_i = 0;
     ...
     * major_count=86,minor_count=24,major_mult=2,minor_inc=86
     */
    ...
    /**
     * Mode2 form1和Mode2 form 2，这两个格式的大小都是0x930
	...
	按ISO/IEC10149的规定，CD-ROM扇区中的ECC码采用GF(28)域上的RSPC码产生172个字节的P校验符号和104个字节的Q校验符号
     */
    public static void main(String[] args) throws Exception {
    	RandomAccessFile file=new RandomAccessFile("xxxx.bin", "r");
    	int baseAddr=0;
    	byte[] data=new byte[2056];
    	file.seek(baseAddr+0x10); 
    	file.read(data);
		byte[] edc=generateErrorDetectionAndCorrection(data, 0, data.length);	//validate subchannel+data 
		System.out.println(Util.hexEncode(edc));
		
		data=new byte[2064];
		file.seek(baseAddr+0x10);
		file.read(data, 4, data.length-4);
		
		byte[] p=new byte[L2_P];
		generateErrorCorrectionCode_P(data,0,p,0);	//validate new byte[4] + subchannel+data +edc
		System.out.println(Util.hexEncode(p));
		
		byte[] diskP=new byte[L2_P];
		file.seek(baseAddr+0x81c);
		file.read(diskP);
		System.out.println(Arrays.equals(p, diskP));
		
		data=new byte[2064+L2_P];
		file.seek(baseAddr+0x10);
		file.read(data, 4, data.length-4);
		
		byte[] q=new byte[L2_Q];
		generateErrorCorrectionCode_Q(data, 0, q, 0);	//validate new byte[4]+ subchannel+data +edc+p
		System.out.println(Util.hexEncode(q));
		
		byte[] diskQ=new byte[L2_Q];
		file.seek(baseAddr+0x81c+p.length);
		file.read(diskQ);
		System.out.println(Arrays.equals(q, diskQ));
